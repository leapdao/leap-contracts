...
contract('ExitToken', (accounts) => {
  ...
  describe('TokenizedExits', function() {
    const p = [];
    let bridge;
    let token;
    let exitToken;
    before(async () => {
      ...
      bridge = await deployBridge(token, 4, alice, this.project);

      exitToken = await this.project.createProxy(ExitToken);
      await sendTransaction(exitToken, 
        'initialize', 
        ['address'],
        [bridge.address]);
    ...
    it('should work with non-native tokens', async () => {
      ...
      await bridge.registerToken(secondToken.address, false);
