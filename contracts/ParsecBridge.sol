...
contract ParsecBridge {
  ...
  uint256 constant maxOpCount = 64; // also length of 1 epoche
  ...
  function ParsecBridge(ERC20 _token, uint256 _parentBlockInterval) public {
    ...
    token = _token;
    Block memory genesis;
    genesis.parent = 0x4920616d207665727920616e6772792c20627574206974207761732066756e21; // I am very angry, but it was fun!
    tipHash = keccak256(genesis.parent, uint64(0), bytes32(0));
    chain[tipHash] = genesis;
  ...
  function join(uint256 amount) public {
    require(operators[msg.sender].stakeAmount + amount <= token.totalSupply().div(20));
   ...
   */  
  function claimReward(bytes32[] coinbase, bytes32[] proof) {
  ...
  function submitBlock(bytes32 prevHash, bytes32 root) public isOperator {
    ...
    if (newHeight > maxHeight) {
      ...
      tipHash = tipHash;
      // todo: mine some tokens
      // todo: prune some blocks
      lastParentBlock = uint64(block.number);
  ...
  function getTip() public constant returns (bytes32 root, address operator) {
    return getBlock(chain[tipHash].height);
  }
