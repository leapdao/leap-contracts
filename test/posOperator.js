    ...
    describe('Slot', () => {
      ...
      describe('Auction', () => {
        it('should prevent submission by unbonded validators', async () => {
          await operator.submitPeriodWithCas(0, p[0], '0x01', '0xc0', {from: alice}).should.be.rejectedWith(EVMRevert);
        });
        ...
        it('should allow to auction slot and submit block', async () => {
          ...
          await operator.submitPeriodWithCas(0, p[0], '0x01', '0xc0', { from: alice }).should.be.fulfilled;
        ...
        it('should allow submission when slot auctioned in same epoch', async () => {
          await operator.submitPeriodWithCas(0, p[1], '0x02', '0xc0', {from: alice}).should.be.fulfilled;
          p[2] = await bridge.tipHash();
        });

        it('should prevent submission by auctioned slot in later epoch', async () => {
          await operator.submitPeriodWithCas(0, p[2], '0x03', '0xc0', {from: alice}).should.be.rejectedWith(EVMRevert);
          await operator.submitPeriodWithCas(0, p[2], '0x03', '0xc0', {from: bob}).should.be.rejectedWith(EVMRevert);
        });
        ...
        it('should allow to activate auctioned slot and submit', async () => {
          // increment Epoch
          await operator.submitPeriodWithCas(1, p[2], '0x03', '0xc0', {from: charlie}).should.be.fulfilled;
          p[3] = await bridge.tipHash();
          await operator.submitPeriodWithCas(1, p[3], '0x04', '0xc0', {from: charlie}).should.be.fulfilled;
          p[4] = await bridge.tipHash();
          await operator.submitPeriodWithCas(1, p[4], '0x05', '0xc0', {from: charlie}).should.be.fulfilled;
          p[5] = await bridge.tipHash();
          await operator.submitPeriodWithCas(1, p[5], '0x06', '0xc0', {from: charlie}).should.be.fulfilled;
          ...
          await operator.submitPeriodWithCas(0, p[6], '0x07', '0xc0', {from: bob}).should.be.fulfilled;
        ...
        it('should prevent submission by logged-out slot in later epoch', async () => {
          // increment epoch
          await operator.submitPeriodWithCas(1, p[7], '0x08', '0xc0', {from: charlie}).should.be.fulfilled;
          p[8] = await bridge.tipHash();
          // try to submit when logged out
          await operator.submitPeriodWithCas(0, p[8], '0x09', '0xc0', {from: bob}).should.be.rejectedWith(EVMRevert);
        });

        it('should allow to withdraw after logout', async () => {
          // increment epoch
          await operator.submitPeriodWithCas(1, p[8], '0x09', '0xc0', {from: charlie}).should.be.fulfilled;
          p[9] = await bridge.tipHash();
          await operator.submitPeriodWithCas(1, p[9], '0x0a', '0xc0', {from: charlie}).should.be.fulfilled;
          p[10] = await bridge.tipHash();
          await operator.submitPeriodWithCas(1, p[10], '0x0b', '0xc0', {from: charlie}).should.be.fulfilled;
